...
public:
    ...
    static T generateInRange(T min, T max) {
        static_assert(std::is_arithmetic<T>::value, "T must be a number type (int, float, etc.)");
    ...
    template <typename T>
    static T generateFromSet(const std::vector<T>& set) {
        if (set.empty())
    ...
    template <typename T>
    static T generateFromSetWithProb(const std::vector<T>& set, const std::vector<float>& probabilities) {
        if (set.size() != probabilities.size() || set.empty())
            throw std::invalid_argument("Set and probabilities must have the same size and cannot be empty");
